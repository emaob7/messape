...
import SearchIcon from '@mui/icons-material/Search'; // Icono para el botón de búsqueda
import html2pdf from 'html2pdf.js'; // Importar html2pdf
import logo from '../assets/logo.png';

const Form1 = ({ selectedRow, userRole, userName }) => {
 ...
 // Función para buscar un registro por número
 const handleSearch = async () => {
  try {
    ...
    // Busca el documento en Firestore usando el campo "numero" como ID
    const docRef = doc(db, 'formularios', formData.numero);
    const docSnap = await getDoc(docRef);

    if (docSnap.exists()) {
      // Si el documento existe, carga los datos en el estado
...
const limpiar = () =>{
   setFormData({
    ...
    observacion: '',
    confidencial: false, // Nuevo campo para indicar si es confidencial
    userName: userName || '', // Reiniciar el campo userName,
  });
}
